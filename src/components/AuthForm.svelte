...
{#if mode === 'signIn'}
  <p>
    Do you not have an account ? <span class="has-text-link link" on:click={() => (mode = 'signUp')}>Sign
      Up</span>
  </p>
{:else}
  <p>
    Do you already have an account ? <span class="has-text-link link" on:click={() => (mode = 'signIn')}>Sign
      In</span>
  </p>
{/if}
